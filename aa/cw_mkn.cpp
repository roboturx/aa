...
Cw_mkn::Cw_mkn(QWidget *parent) : QWidget(parent)
{
    ...
    // verileri view de gösterebilmek için modelleyelim
    Cw_mkn::mdll_mkcins();
    Cw_mkn::mdll_mkmark();
    Cw_mkn::mdll_mkmodl();
    Cw_mkn::mdll_mkn();
...
void Cw_mkn::mkn_ui()   // centralwidget görüntüsü
{
...
void Cw_mkn::mdll_mkn()
{
    ...
    /// mapper MKN
    mppMKN = new QDataWidgetMapper(this);
    mppMKN->setModel(MKNmodel);

    // mapperdaki comboboxları çalıştır
    mppMKN->setItemDelegate(new QSqlRelationalDelegate(this));


    led_mknKurumno = new QLineEdit();
    mppMKN->addMapping(led_mknKurumno, MKNmodel->fieldIndex("mkn_kurumNo"));
    led_mknPlaka = new QLineEdit();
    mppMKN->addMapping(led_mknPlaka, MKNmodel->fieldIndex("mkn_plaka"));
    ///qDebug()<< "combo cins";
    ///
    ...
    mppMKN->addMapping(cbx_mknCins, MKNmodel->fieldIndex("mkn_cinsi"));
    ...
    mppMKN->addMapping(cbx_mknMark, MKNmodel->fieldIndex("mkn_marka"));
    ...
    mppMKN->addMapping(cbx_mknModl, MKNmodel->fieldIndex("mkn_modeli"));
    ...
    mppMKN->addMapping(cbx_mknSurucu, MKNmodel->fieldIndex("mkn_surucu"));
...
void Cw_mkn::MKNtview_view()
{
    ...
    MKNtview->setModel(MKNmodel);

    // tableview de comboboxları göster
    //MKNtview->setItemDelegate(new MakinaDelegate(MKNtview));
    MKNtview->setItemDelegate(new QSqlRelationalDelegate(this));
    MKNtview->setSelectionMode(QAbstractItemView::SingleSelection);
    MKNtview->setSelectionBehavior(QAbstractItemView::SelectItems);
    MKNtview->setContextMenuPolicy(Qt::CustomContextMenu);
    MKNtview->setCurrentIndex(MKNmodel->index(0,0));
    MKNtview->selectionModel()->setCurrentIndex(MKNtview->currentIndex (),
                                                QItemSelectionModel::SelectCurrent);


    MKNtview->setColumnHidden(MKNmodel->fieldIndex("id_mkn"), true);
    MKNtview->setColumnHidden(MKNmodel->fieldIndex("mkn_resim"), true);
    MKNtview->setColumnHidden(MKNmodel->fieldIndex("mkn_rating"), true);
...
void Cw_mkn::mkn_kontrol()
{
    QModelIndex idxMKN = MKNtview->currentIndex();
    ...
    connect(MKNtview, SIGNAL(customContextMenuRequested(QPoint)),
    ...
    //        mppMKN, SLOT(setCurrentModelIndex(QModelIndex)));
    ...
    //connect(mppMKN, &QDataWidgetMapper::currentIndexChanged,
...
void Cw_mkn::onMKNtview_sagmenuSLOT(QPoint pos)
{
    ...
    connect (ekleAct_mkn, &QAction::triggered, this,
    ...
    menu->popup(MKNtview->viewport()->mapToGlobal(pos));
...
void Cw_mkn::onmnMKN_resimEklE_clickedSLOT()
{
    ...
    if ( file.open(QIODevice::ReadOnly))
    {
        ...
        QModelIndex index = MKNtview->currentIndex();
        int row = index.row() ;
...
void Cw_mkn::onmnMKN_arac_sil_clickedSLOT()
{
    ...
    if (msgBox.clickedButton() == pb_tmm)
    {
        QSqlQuery q_qry;
        QString s_qry;
        QModelIndex mkn_indx = MKNtview->currentIndex ();
...
void Cw_mkn::onMKNtview_IE_filterSLOT(QModelIndex mknIndex )
{
    if (mknIndex.isValid())
    {
        ...
        QString bslk = mknrec.value("mkn_mkn_kurumNo").toString()+" - "+
...
*/
        ...
        //    qDebug () <<"mkn row changggggedd 10";
        mdl_mkmodl->select();
    }
    else
    {
...
void Cw_mkn::onMKNtview_resimGosterSLOT(QModelIndex)
{
    ...
    QByteArray outByteArray = MKNtview->
...
{
    // makina stok ekle
    qDebug()<<"mknekle 1"<<endl<<"mkntview index"<<MKNtview->currentIndex ()<<endl<<
              "mppmkn index"<<mppMKN->currentIndex ();
    ...
    mppMKN->addMapping(spn_mknYil, MKNmodel->fieldIndex("mkn_yil"));
    ...
    mppMKN->addMapping(led_mknSase, MKNmodel->fieldIndex("mkn_saseno"));
    ...
    mppMKN->addMapping(led_mknMotor, MKNmodel->fieldIndex("mkn_motorno"));
    ...
    mppMKN->addMapping(led_mknMtip, MKNmodel->fieldIndex("mkn_motortip"));
    ...
    mppMKN->addMapping(cbx_mknYkt, MKNmodel->fieldIndex("mkn_yakit"));
    ...
    mppMKN->addMapping(clndr_mknSurucutar, MKNmodel->fieldIndex("mkn_surucu_tar"));
    ...
    mppMKN->addMapping(cbx_mknBirim, MKNmodel->fieldIndex("mkn_birim"));
    ...
    mppMKN->addMapping(cbx_mknByer, MKNmodel->fieldIndex("mkn_byer"));
    ...
    mppMKN->addMapping(ted_mknAcklm, MKNmodel->fieldIndex("mkn_aciklama"));

    lB_resim = new QLabel;
    lB_resim->setFixedSize(200,110);
    mapperL->addWidget(lB_resim   ,++satr,5,1,4,nullptr);
    mppMKN->addMapping(lB_resim , MKNmodel->fieldIndex("mkn_resim"));
    rsm_ByteArray = new QByteArray;
    *rsm_ByteArray = "";

    //        mppMKN->addMapping(*rsm_ByteArray, MKNmodel->fieldIndex("resim"));
    //mppMKN->addMapping(led_rating, MKNmodel->fieldIndex("rating"));


    //    /// yeni ekle penceresine girerken index ne
    //    QModelIndex mknndx = MKNtview->currentIndex ();
    //    //mppMKN->toFirst ();
    //    mppMKN->setCurrentIndex( mknndx.row ()) ;
    int sutn=0;
    //   satr +=2;
    ...
    connect( pb_ilk, &QPushButton::clicked,
             mppMKN, &QDataWidgetMapper::toFirst );
...
void Cw_mkn::MKNekle()
{
    ...
    s_q="INSERT INTO dbtb_mkn "
        ...
        "  mkn_modeli  , mkn_yil     , mkn_saseno, mkn_motorno   ,"
