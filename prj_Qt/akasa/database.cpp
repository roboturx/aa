...
QSqlDatabase DataBase::DBconnect()
{
   ...
   // return true;
    ...
    qDebug() << m_dbPath;
    qDebug() << m_dbName;
    if (!QFile(m_dbPath+m_dbName).exists()) {
...
bool DataBase::DBTableCreate(const QString tableName, const QString tableStr)
{
...
/*
        ...
        QList<QString> ekle;
        // sql statement
        ekle << "INSERT INTO tb_konum (f_konumID, f_konumAdi, f_konumAdres) "
                "VALUES (:A , :B , :C)";
        // field sayısı
        ekle << "3";
        // bind values
...
bool DataBase::create_Table_person()
{
    ...
    QSqlQuery query;
    if (!query.exec("CREATE TABLE IF NOT EXISTS tb_konum "
        ...
        qDebug() << "               CREATED. ";
        qDebug() << "               inserting RECORDS into tb_konum ";
...
bool DataBase::create_Table_isemri()
{
    ...
    if (!query.exec("CREATE TABLE IF NOT EXISTS `isemri` ("
                    ...
                    "companyID INTEGER,"
                    "name TEXT,"
                    "FOREIGN KEY(companyID ) REFERENCES tb_konum(projectID))")) {
        qDebug() << "               NOT CREATED ";
        qDebug() << query.lastError().text();
        return false;
    } else {
