...
bool DataBase::insertIntoHesapTable() //const QVariantList &data
{
    ...
    query.prepare("INSERT INTO tb_hsp"
            "(h_tip, h_kod, h_ad, h_prnt, h_acklm, h_prbrm, h_prbrmx, h_ana, h_gzli ) "
     "VALUES (:A,    :B,    :C,   :D,     :E,      :F,      :G,       :H,    :J     )");

    query.bindValue(":A", "hesap tipi");        // tip
    query.bindValue(":B", 1);        // kod
    query.bindValue(":C", "hesap adı");         // ad
    query.bindValue(":D", 0);         // parent root hesap
    query.bindValue(":E", "açıklama");          // açıklama
    query.bindValue(":F", "para birimi");       // para birimi
    query.bindValue(":G", "para birim kesiri"); // para birim kesir
    query.bindValue(":H", true);                // ana hesap mı
    query.bindValue(":J", false);               // gizli mi

    QSqlQuery query2;
    query2.prepare("INSERT INTO tb_hsp ("
                   "h_tip, h_kod, h_ad, h_prnt, "
                   "h_acklm, h_prbrm, h_prbrmx, "
                   "h_ana, h_gzli ) "
                   "VALUES (:A, :B, :C, :D, :E, :F, :G, :H, :J  )");

    query2.bindValue(":A", "hesap tipi");        // tip
    query2.bindValue(":B", 101);                  // kod
    query2.bindValue(":C", "hesap adı");         // ad
    query2.bindValue(":D", 1);              // parent
    query2.bindValue(":E", "açıklama");          // açıklama
    query2.bindValue(":F", "para birimi");       // para birimi
    query2.bindValue(":G", "para birim kesiri"); // para birim kesir
    query2.bindValue(":H", true);                // ana hesap mı
    query2.bindValue(":J", false);               // gizli mi
    ...
    if (!query.exec())
    {
        ...
        if (!query2.exec()) {
            qDebug() << "       error insert second into HESAP";
            qDebug() << query2.lastError().text();
            return false;
        }
        else
        {
            qDebug() << "       second record inserted to HESAP";
            return true;
        }
        return false;
    }
