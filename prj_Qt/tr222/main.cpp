...
int main(int argc, char *argv[])
{
   ...
   // db.insertIntoHesapTable ();
  //  db.insertIntoYevmiyeTable ();

    QStandardItemModel model;
    //QSqlQuery  query("SELECT Hsp_parentkod, Hsp_name, Hsp_kodu from Tb_Hsp");

    QSqlQuery query("SELECT h_name, h_parentkod, h_kod FROM Tb_Hsp "
                       "ORDER BY h_kod ASC");
    const QSqlRecord rec = query.record();
    while (query.next()) {
        QString Hsp_name = query.value(rec.indexOf("h_name")).toString();
        int Hsp_parentkod = query.value(rec.indexOf("h_parentkod")).toInt();
        int Hsp_kodu = query.value(rec.indexOf("h_kod")).toInt();
