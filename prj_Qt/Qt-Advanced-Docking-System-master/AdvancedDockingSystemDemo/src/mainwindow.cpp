...
static ADS_NS::SectionContent::RefPtr createLongTextLabelSC(ADS_NS::ContainerWidget* container)
{
	...
	l->setAlignment(Qt::AlignTop | Qt::AlignLeft);
        l->setText(QString("Burası mw de ADS_NS nin il label ı"));
	bl->addWidget(l);
	...
	ADS_NS::SectionContent::RefPtr sc = ADS_NS::SectionContent::newSectionContent(QString("uname-%1").arg(index), container, new IconTitleWidget(QIcon(), QString("Label %1").arg(index)), w);
        sc->setTitle("İLK Label " + QString::number(index));
	return sc;
}
...
static ADS_NS::SectionContent::RefPtr createCalendarSC(ADS_NS::ContainerWidget* container)
{
	...
	const int index = ++CONTENT_COUNT;
        return ADS_NS::SectionContent::newSectionContent(QString("takvim -%1").arg(index), container, new IconTitleWidget(QIcon(), QString("Calendar %1").arg(index)), w);
}
...
static ADS_NS::SectionContent::RefPtr createFileSystemTreeSC(ADS_NS::ContainerWidget* container)
{
	...
	const int index = ++CONTENT_COUNT;
        return ADS_NS::SectionContent::newSectionContent(QString("dosya sistemi-%1").arg(index), container, new IconTitleWidget(QIcon(), QString("Filesystem %1").arg(index)), w);
}
	...
	ui(new Ui::MainWindow)
{
	...
	// Setup actions.
        QObject::connect(ui->actionContentList, SIGNAL(triggered()),
                         this, SLOT(showSectionContentListDialog()));

	// ADS - Create main container (ContainerWidget).
	_container = new ADS_NS::ContainerWidget();
#if QT_VERSION >= 0x050000
        QObject::connect(_container, &ADS_NS::ContainerWidget::activeTabChanged,
                         this, &MainWindow::onActiveTabChanged);
        QObject::connect(_container, &ADS_NS::ContainerWidget::sectionContentVisibilityChanged,
                         this, &MainWindow::onSectionContentVisibilityChanged);
#else
        QObject::connect(_container, SIGNAL(activeTabChanged(const SectionContent::RefPtr&, bool)),
                         this, SLOT(onActiveTabChanged(const SectionContent::RefPtr&, bool)));
        QObject::connect(_container, SIGNAL(sectionContentVisibilityChanged(SectionContent::RefPtr,bool)),
                         this, SLOT(onSectionContentVisibilityChanged(SectionContent::RefPtr,bool)));
#endif
	...
	if (true)
	{
		...
		ADS_NS::SectionWidget* sw = NULL;
                ...
                sw = _container->addSectionContent(createLongTextLabelSC(cw),
                                                   sw, ADS_NS::CenterDropArea);
                sw = _container->addSectionContent(createCalendarSC(cw),
                                                   sw, ADS_NS::RightDropArea);
                sw = _container->addSectionContent(createFileSystemTreeSC(cw),
                                                   sw, ADS_NS::CenterDropArea);
		...
		ADS_NS::SectionContent::RefPtr sc = createLongTextLabelSC(cw);
                //sc->setFlags(ADS_NS::SectionContent::AllFlags
                //             ^ ADS_NS::SectionContent::Closeable);
                sc->setFlags(ADS_NS::SectionContent::Closeable);
