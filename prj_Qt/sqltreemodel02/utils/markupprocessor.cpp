...
MarkupProcessor::MarkupProcessor(const QString& text) :
    m_text( text ),
 //   m_flags( flags ),
    m_index( 0 ),
    m_matched( false ),
...
void MarkupProcessor::parseText()
{
    ...
    for ( ; ; ) {
        switch ( m_token ) {
            case T_TEXT: {
//                QRegularExpression subtokenExp( "\\*\\*+|__+" );
//                int pos = 0;
//                for ( ; ; ) {
//                    int oldpos = pos;
...
//                    if ( pos < 0 ) {
//                        text ="ffff" /*HtmlText::convertTabsToSpaces( m_value.mid( oldpos ), column )*/;
//                        m_result += "fffffrt" /*HtmlText::parse( text, m_flags ).toString()*/;
//                        break;
//                    }
...
//                next();
            ...
            case T_BACKTICK:
                text = "hghhh"/*HtmlText::convertTabsToSpaces( m_value, column )*/;
...
{
    if ( m_token == T_TEXT ) {
        QRegularExpression listExp( "[ \\t]*(\\*{1,6})[ \\t](.*)" );
//        if (  1 /*listExp.match( m_value , QRegularExpression::CaseInsensitiveOption )*/) {
//            m_value = listExp.match(m_value );// .cap( 2 );
//            if ( m_value.isEmpty() )
//                next();
            return 1 /*listExp.cap( 1 ).length()*/;
        //}
    }
    return 0;
}
