...
public:
   ...
   The provided parameter is a QList of values to be used as default
   values in newly added records. Item 0 is placed into record field
   0, item 1 in field 1 and so on. If the list is too short, then
   remaining fields remain uninitialized, equally those items that
   contain an invalid QVariant (i.e., QVariant()). A too long list
   does not harm, extra list items are  silently ignored.

   If using QSqlRelationalTableModel as underlying model:
   all foreign key fields must be initialized to a valid entry
   (that is present in the related table; this is a requirement
   steming from QSqlRelationalTableModel.

   If the item corresponding to the id field is not an invalid
   QVariant, then the provided value will be used to initialize
   the id field; as is the cas for all other field. This has
   implications:
   only 1 row may be inserted per call to insertRows(), and between
   calls to insertRows() a new id value needs to be set by
   setDefaultValue. That only 1 row may be inserted at a time has
   further consequences: drag and drop functionality is limited.
   Thus, this is not a recommended method to provide unique keys for
   most applications; consequently, the item corresponding to the
   id field will most often conatin QVarient().
   */
   ...
   bool setData(const QModelIndex &index,
    ...
    inherited functionality is sufficient */
   ...
   /* other inherited virtual functions, for which I see no need to
    * re-implement
